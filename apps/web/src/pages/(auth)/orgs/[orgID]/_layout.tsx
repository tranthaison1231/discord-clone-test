...
import { Calendar, ChevronDown, Home, Users } from "lucide-react";
...
export default function Org() {
  ...
  const handleNavigate = (id: string) => {
    if (id) {
      navigate(`/orgs/${param.orgID}/channels/${id}`);
    }
  };
  return (
    <div className="w-full flex">
      <div className="bg-primary-foreground/80 w-1/6 h-full">
        ...
        <div className="pt-4 pl-2 space-y-4 items-center text-primary/60">
          {Catagorys.map((catagory) => (
            <div
              key={catagory.id}
              className="flex gap-2 px-3 py-2 cursor-pointer hover:bg-primary-foreground/90"
            >
              {catagory.id === "2" ? (
                <svg
                  width="24"
        ...
        <div className="">
          <div className="uppercase pl-2 flex items-center gap-1">
            {" "}
            <ChevronDown className=" w-3 h-3 " />
            Class
          </div>
          <div className="pl-2 text-primary/60">
            {ListClass.map((item) => (
              <div
                key={item.id}
                className="flex gap-2 px-3 py-2 cursor-pointer hover:bg-primary-foreground/90"
